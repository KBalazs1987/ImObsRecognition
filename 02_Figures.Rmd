...
fig1_4 <- ggplot() + 
  ...
  geom_density(mapping = aes(x = expperc, fill = "Non-immunogenic"), data = ligands_nonhuman_d1[ligands_nonhuman_d1$immunogenicity == 0,]) +
...
fig1_5 <- ggplot() + 
  ...
  geom_density(mapping = aes(x = expperc, fill = "Non-immunogenic"), data = ligands_nonhuman_d2[ligands_nonhuman_d2$immunogenicity == 0,]) +
  coord_cartesian(ylim = c(.5,1.25), xlim = c(0,1)) +
  geom_vline(xintercept = c(.15,.75), linetype = "dashed") +
  scale_fill_manual(values = alpha(c("#DC0000B2","#3C5488B2"),.3)) +
  labs(tag = " ", x = "Percentile of TCEM expression", y = "Density", fill = "Group") +
...
groups = aggregate(ligands_nonhuman$immunogenicity ~ ligands_nonhuman$bl62score_group, FUN = function(x)  {
...
```{r}
load("D:/CloudStation/pentamer_project/articleplots_objects/pathogen_fract")
...
```{r}
load("D:/CloudStation/pentamer_project/articleplots_objects/pathogen_heatmapmtx")
...
#Not trigger
pss_nt = parSeqSim(protlist = as.list(ligands_nonhuman$epitope[ligands_nonhuman$immunogenicity == 0]), cores = 6, submat = "BLOSUM62")
#save(pss_nt_d2, file = "objects/pss_nt_figs2")
pss_nt_values = pss_nt[upper.tri(pss_nt)]
l1 = length(pss_nt_values)

#Trigger
pss_t = parSeqSim(protlist = as.list(ligands_nonhuman$epitope[ligands_nonhuman$immunogenicity == 1]), cores = 6, submat = "BLOSUM62")
...
dataset_1 = cbind(ligands_nonhuman[,c("allele", "epitope", "immunogenicity", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d1")
...
dataset_2 = cbind(ligands_nonhuman[,c("allele", "epitope", "immunogenicity", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d2")
...
ligands_nonhuman %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman$immunogenicity)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
plotdata = plsmo(x = ligands_nonhuman$expperc, y = ligands_nonhuman$immunogenicity)
plotdata = data.frame(x = plotdata$`1`$x, y = plotdata$`1`$y)
...
figS9_4 <- ggplot() + 
  geom_density(mapping = aes(x = expperc, fill = "Immunogenic"), data = ligands_nonhuman[ligands_nonhuman$immunogenicity == 1,]) +
  geom_density(mapping = aes(x = expperc, fill = "Non-immunogenic"), data = ligands_nonhuman[ligands_nonhuman$immunogenicity == 0,]) +
  coord_cartesian(ylim = c(.5,1.25), xlim = c(0,1)) +
  geom_vline(xintercept = c(.15,.75), linetype = "dashed") +
  scale_fill_manual(values = alpha(c("#DC0000B2","#3C5488B2"),.3)) +
  annotate(geom = "text", x = .91, y = 1.2, label = "DS1", color = "black", size = 2) +
