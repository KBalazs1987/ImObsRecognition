...
```{r}
load("ligands_nonhuman_d1d2") #github
...
```{r}
load("ligands_nonhuman_d1") #github
...
```{r}
load("ligands_nonhuman_d2") #github
...
#ggsave(fig1, filename = "D:/Fig1.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
```
...
```{r}
load("sars_cov_similarity") #github
...
tbl = read.delim("FIMMUN.txt", stringsAsFactors = F) #github
...
```{r}
fig4a = readPNG("Cross_reactivity_final.png") #github
...
load("alleles_for_heatmap") #github
load("np_tcems_hla_pathogens") #github
res %<>% 
...
# save(fract, file = "pathogen_fract")
# save(heatmapmtx,file = "pathogen_heatmapmtx")
# save(res, file = "pathogen_res")
```
...
```{r}
load("pathogen_heatmapmtx") #github
...
```{r}
load("infection_risk_HLA") #github
...
dropped_ep = readLines("dropped_eps_clo_d2.txt") #github
load("all_iedb") #github
...
```{r}
load("objects/ligands_nonhuman_d1") #github
...
load("objects/ligands_nonhuman_d2") #github
...
```{r}
load("ligands_nonhuman_d1d2_wosecanc") #github
wilcox.test(freqpenta_tcem ~ immunogenicity, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(freqpenta_tcem ~ immunogenicity, subset(ligands_nonhuman, dataset == "d2"))
...
```{r}
load("ligands_nonhuman_d1_wosecanc") #github
ligands_nonhuman_d1 %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman_d1$immunogenicity)
...
```{r}
load("ligands_nonhuman_d2_wosecanc") #github
ligands_nonhuman_d2 %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman_d2$immunogenicity)
...
```{r}
chowell = read.delim("Chowell_st_ie.txt", sep = "\t", stringsAsFactors = F) #github
load("ligands_nonhuman_d1d2") #github
...
#A6
tbl = read.delim("A6_position_depletion_matrix.csv", sep = ",", stringsAsFactors = F) #github
