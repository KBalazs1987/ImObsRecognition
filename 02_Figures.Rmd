...
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d2"))

table(ligands_nonhuman$group)

fig1_1 = ggplot(ligands_nonhuman, aes(x = group, y = freqpenta_tcem, color = trig, fill = trig)) + 
...
table(ligands_nonhuman$trig)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
plotdata = plsmo(x = ligands_nonhuman$expperc, y = ligands_nonhuman$trig)
...
fig1_4 <- ggplot() + 
  geom_density(mapping = aes(x = expperc, fill = "Immunogenic"), data = ligands_nonhuman[ligands_nonhuman$trig == 1,]) +
  geom_density(mapping = aes(x = expperc, fill = "Non-immunogenic"), data = ligands_nonhuman[ligands_nonhuman$trig == 0,]) +
...
fig1_45
rm(fig1_4, fig1_5, ligands_nonhuman)
```
...
table(ligands_nonhuman$group)
wilcox.test(thymopscore ~ trig, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(thymopscore ~ trig, subset(ligands_nonhuman, dataset == "d2"))

fig1_6 = ggplot(ligands_nonhuman, aes(x = group, y = thymopscore, color = trig, fill = trig)) + 
  geom_boxplot(outlier.shape = NA, lwd = .5, width = .6) +
  scale_x_discrete(labels = c("+","-","+","-")) +
...
wilcox.test(immunopscore ~ trig, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(immunopscore ~ trig, subset(ligands_nonhuman, dataset == "d2"))

fig1_7 = ggplot(ligands_nonhuman, aes(x = group, y = immunopscore, color = trig, fill = trig)) + 
  geom_boxplot(outlier.shape = NA, lwd = .5, width = .6) +
  scale_x_discrete(labels = c("+","-","+","-")) +
...
#ggsave(fig1, filename = "main_plots_rs/Fig1_25.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
#ggsave(fig1, filename = "main_plots_rs/Fig1_25.png", width = 22, height = 10, units = "cm", dpi = "retina")
```
...
ligands_nonhuman$freqpenta_tcem_group = cut(x = ligands_nonhuman$freqpenta_tcem, breaks = c(min(ligands_nonhuman$freqpenta_tcem),median(ligands_nonhuman$freqpenta_tcem),max(ligands_nonhuman$freqpenta_tcem)), labels = F, include.lowest = T, right = F) #1: <median, 2: >= median. Median = 4.
table(ligands_nonhuman$freqpenta_tcem_group)
fisher.test(ligands_nonhuman$trig, ligands_nonhuman$freqpenta_tcem_group)
...
fisher.test(ligands_nonhuman$trig[ligands_nonhuman$pentamer_expression_group %in% c(1,2)], ligands_nonhuman$pentamer_expression_group[ligands_nonhuman$pentamer_expression_group %in% c(1,2)])
fisher.test(ligands_nonhuman$trig[ligands_nonhuman$pentamer_expression_group %in% c(2,3)], ligands_nonhuman$pentamer_expression_group[ligands_nonhuman$pentamer_expression_group %in% c(2,3)])
#1/1.347172
#1/1.818366
...
table(ligands_nonhuman$thymopscore_group)
fisher.test(ligands_nonhuman$trig, ligands_nonhuman$thymopscore_group)
...
table(ligands_nonhuman$immunopscore_group)
fisher.test(ligands_nonhuman$trig, ligands_nonhuman$immunopscore_group)
```
...
```{r}
load("../articleplots_objects/stat_individual")

df_tcemfreq = data.frame(fraction = c(stat_individual[,"tcemfreq_0_3"], (stat_individual[,"tcell_tcemfreq_4_7"] + stat_individual[,"tcell_tcemfreq_8_inf"]) / (stat_individual[,"bound_tcemfreq_4_7"] + stat_individual[,"bound_tcemfreq_8_inf"])), group = rep(c("0-3","4-"), each = nrow(stat_individual)), IDs = rep(rownames(stat_individual), 2))
...
#ggsave(fig2, filename = "Fig2_9.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
...
#Fig3
##A Similarity and TCEM frequency
Dataset 1 és 2

```{r}
load("ligands_nonhuman_d1d2")
ligands_nonhuman$freqpenta_tcem_group = cut(x = ligands_nonhuman$freqpenta_tcem, breaks = c(min(ligands_nonhuman$freqpenta_tcem),median(ligands_nonhuman$freqpenta_tcem),max(ligands_nonhuman$freqpenta_tcem)), include.lowest = T, right = F)
...
```{r}
load("ligands_nonhuman_d1d2")
#hist(ligands_nonhuman$bl62score)
ligands_nonhuman$bl62score_group = cut(ligands_nonhuman$bl62score, 25)
table(ligands_nonhuman$bl62score_group)

groups = aggregate(ligands_nonhuman$trig ~ ligands_nonhuman$bl62score_group, FUN = function(x)  {
  temp = table(factor(x, levels = c("1", "0")))
  temp["1"]/temp["0"]
  })
colnames(groups) = c("group_int", "immunogenicity")
groups$group_int = as.character(groups$group_int)
groups$group_mean = sapply(groups$group_int, function(x) {
  mean(c(as.numeric(gsub("\\(", "", strsplit(as.character(x), ",")[[1]][1])), as.numeric(gsub("\\]", "", strsplit(as.character(x), ",")[[1]][2]))))
})
colorbg = data.frame(startlocx = c(.59, .72), 
                        endlocx = c(.72, .89),
                       col = c("#3C5488B2","#DC0000B2"))

fig3b = ggplot() + 
  geom_point(data = groups, mapping = aes(group_mean, immunogenicity), size = 1) +
  geom_smooth(mapping = aes(group_mean, immunogenicity), data = groups, se = F, linetype = "dashed", col = "black", size = 0.5, span = 0.4) +
  geom_rect(data = colorbg, mapping = aes(xmin = startlocx, xmax = endlocx, ymin=0, ymax=2), fill = colorbg$col, alpha = 0.3) +
  geom_vline(xintercept = groups$group_mean[which.max(groups$immunogenicity)], linetype = "dashed", color = "black") +
  labs(tag = "B", x = "Mean of sequence similarity values in the group", y = "Ratio of immunogenic and non-immunogenic peptides") +
  theme_bw() +
  theme(
    axis.title.x = element_text(size = 8),
    axis.title.y = element_text(size = 8),
...
groups$group_mean[which.max(groups$immunogenicity)]
cor.test(groups$group_mean[1:which.max(groups$immunogenicity)], groups$immunogenicity[1:which.max(groups$immunogenicity)], method = "spearman")
cor.test(groups$group_mean[which.max(groups$immunogenicity):nrow(groups)], groups$immunogenicity[which.max(groups$immunogenicity):nrow(groups)], method = "spearman")

rm(groups, ligands_nonhuman, colorbg)
```

##C COVID

```{r}
load("../objects/sars_cov_simil_to_balazs")
sars_simil = data.frame(to_balazs, stringsAsFactors = F)
rm(to_balazs)
...
ggsave(fig3, filename = "main_plots_rs/Fig3_5.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
rm(lay, margin)
```
...
tbl = read.delim("../crossreact/FIMMUN.txt", stringsAsFactors = F)
...
```{r}
load("crossreact/simmatrix_nimtcems43")
...
fig4a = readPNG("main_plots_rs/Cross_reactivity_final.png")
fig4a = rasterGrob(fig4a)

fig4 = grid.arrange(fig4a, fig4b, fig4c, fig4d, nrow = 2, ncol = 2)
ggsave(fig4, filename = "main_plots_rs/Fig4_20.pdf", width = 18, height = 18, units = "cm", dpi = "retina")
```
...
```{r}
#"UP000464024" = "sarsvov2"
species = list.files("../objects/uniprot_proteomes_hla_binding/eps/")
load("../objects/pentamerfreq_tcem")
load("../objects/score_list_tpr_normalized_with_division_median_two_sides")
load("../objects/expr_list_tcem")
...
fract = t(sapply(species, FUN = function(x) {
  load(paste0("../objects/uniprot_proteomes_hla_binding/eps/", x))
...
load("../articleplots_objects/results_for_heatmaps")
alleles = sort(unique(res$alleles))
load("../articleplots_objects/results_for_heatmaps_medians_locspec_sarsexpr_033_sarsthymo_025_previous_corrected.jpg")
res %<>% 
...
save(fract, file = "../articleplots_objects/pathogen_fract")
save(heatmapmtx,file = "../articleplots_objects/pathogen_heatmapmtx")
save(res, file = "../articleplots_objects/pathogen_res")
```
...
```{r}
load("../articleplots_objects/pathogen_fract")
...
```{r}
load("../articleplots_objects/pathogen_heatmapmtx")
...
##D Protective and susceptible alleles

```{r}
load("../articleplots_objects/groups_sars_extended_expr_33_thymo_25.jpg")
...
ggsave(fig5, filename = "main_plots_rs/Fig5_50.pdf", width = 18, height = 22, units = "cm", dpi = "retina")
```

#FigS2
##DIVERSITY, BLOSUM distance density
Sequences of T cell activating and non-activating epitopes have similar diversity and amino acid constitution.

```{r}
library(Biostrings)
library(foreach)
library(doParallel)
load("objects/ligands_nonhuman_d2")

#Not trigger
pss_nt = parSeqSim(protlist = as.list(ligands_nonhuman$epitope[ligands_nonhuman$trig == 0]), cores = 6, submat = "BLOSUM62")
#save(pss_nt_d2, file = "objects/pss_nt_figs2")
pss_nt_values = pss_nt[upper.tri(pss_nt)]
l1 = length(pss_nt_values)

#Trigger
pss_t = parSeqSim(protlist = as.list(ligands_nonhuman$epitope[ligands_nonhuman$trig == 1]), cores = 6, submat = "BLOSUM62")
#save(pss_t_d2, file = "objects/pss_t_d2")
pss_t_values = pss_t[upper.tri(pss_t)]
l2 = length(pss_t_values)

pss = data.frame(nt = c(pss_nt_values, rep(NA, l2-l1)), t = pss_t_values)
save(pss, file = "objects/pss_figs2")
rm(ligands_nonhuman, pss_nt, pss_t, l1, l2, pss_nt_values, pss_t_values)

load("objects/pss_figs2")
Sys.time()
figS2a <- ggplot(pss, aes(x=x)) +
  # Top
  geom_density(aes(x = t, y = ..density..), fill = "#DC0000B2") +
  geom_label(aes(x = 0.45, y = 4, label = "Immunogenic"), color = "#DC0000B2", size = 4) +
  # Bottom
  geom_density(aes(x = nt, y = -..density..), fill = "#3C5488B2") +
  geom_label(aes(x = 0.45, y = -4, label = "Non-immunogenic"), color = "#3C5488B2", size = 4) +
  ...
  theme(
    ...
    axis.line = element_line(colour = "black"))
figS2a
save(figS2a, file = "supp_plots_rs/figS2a_02")
Sys.time()
rm(pss)
```
...
library(FactoMineR)
load("objects/ligands_nonhuman_d2")
ligands_nonhuman_ds_2 = ligands_nonhuman[nchar(ligands_nonhuman$epitope) == 9,]
rm(ligands_nonhuman)
dropped_ep = readLines("01out/dropped_eps_clo_d2.txt")
load("objects/all_iedb")

length(unique(c(ligands_nonhuman_ds_2$epitope, dropped_ep[nchar(dropped_ep) == 9])))
length(ligands_nonhuman_ds_2$epitope)
...
df_full = rbind.data.frame(cbind.data.frame(df_iedb_full[!is.na(fmatch(all_iedb, c(ligands_nonhuman_ds_2$epitope, dropped_ep))),], group = "Full peptide - Before filtering"),cbind.data.frame(df_iedb_full[!is.na(fmatch(all_iedb, ligands_nonhuman_ds_2$epitope)),], group = "Full peptide - After filtering"))
rm(df_iedb_full)
...
df_tcem = rbind.data.frame(cbind.data.frame(df_iedb_tcem[!is.na(fmatch(all_iedb, c(ligands_nonhuman_ds_2$epitope, dropped_ep))),], group = "TCEM - Before filtering"), cbind.data.frame(df_iedb_tcem[!is.na(fmatch(all_iedb, ligands_nonhuman_ds_2$epitope)),], group = "TCEM - After filtering"))
rm(df_iedb_tcem, all_iedb, ligands_nonhuman_ds_2, dropped_ep)
...
save(figS2b, figS2c, file = "supp_plots_rs/figS2bc")
rm(df_full, df_tcem)
```
...
```{r}
load("supp_plots_rs/figS2a_02")
load("supp_plots_rs/figS2bc")
...
ggsave(figS2, filename = "supp_plots_rs/FigS2_17.png", width = 22, height = 13, units = "cm", dpi = "retina")
Sys.time()
rm(lay, margin1)
```

#FigS3 - TCEMDISTRIB
frequency distribution in the human proteome
##Object

```{r}
load("objects/ligands_nonhuman_d1")
...
ggplot(tcems_f, aes(x = freqpenta_tcem, color = tcemsource, fill = tcemsource)) +
  ...
  theme(
    ...
    strip.background = element_blank(),
    strip.placement = "outside")
ggsave("supp_plots_rs/FigS3_5.jpg", width = 14, height = 14, units = "cm", dpi = "retina")
...
figs4_ms = grid.arrange(grobs = c(lapply(list(figs_ms_1), "+", margin1), 
                              lapply(list(figs_ms_2), "+", margin2)), layout_matrix = lay)
ggsave(figs4_ms, filename = "supp_plots_rs/figS4_ms_13.png", width = 20, height = 13, units = "cm", dpi = "retina")
```

#FigS5 - Housekeeping ratio
...
ggsave(figS5, filename = "supp_plots_rs/FigS5_12.jpg", width = 22, height = 11, units = "cm", dpi = "retina")
```

#FigS7 logreg, Table S1

```{r}
ds = "d1"
ds = "d2"

load(paste0("objects/ligands_nonhuman_", ds))
ligands_nonhuman$trig = as.numeric(as.character(ligands_nonhuman$trig))
ligands_nonhuman$pentamer_expression_group = cut(x = ligands_nonhuman$pentamer_expression, breaks = c(min(ligands_nonhuman$pentamer_expression,na.rm=T),quantile(ligands_nonhuman$pentamer_expression,c(0.15,0.75),na.rm=T),max(ligands_nonhuman$pentamer_expression,na.rm=T)), labels = F, include.lowest = T, right = F)
table(ligands_nonhuman$pentamer_expression_group)
ligands_nonhuman %<>% 
  ...
  select(allele, epitope, trig, freqpenta_tcem, pentamer_expression_group2, thymopscore) %>% 
  mutate(log_freqpenta_tcem = log(freqpenta_tcem)) %>% 
  na.omit()

modelglm = glm(formula = trig ~ log_freqpenta_tcem, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef["log_freqpenta_tcem", c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m1"))

modelglm = glm(formula = trig ~ pentamer_expression_group2, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef["pentamer_expression_group2medium", c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m2"))

modelglm = glm(formula = trig~thymopscore, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef["thymopscore", c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m3"))

modelglm = glm(formula = trig~log_freqpenta_tcem + pentamer_expression_group2, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef[c("log_freqpenta_tcem","pentamer_expression_group2medium"), c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m4"))

modelglm = glm(formula = trig~log_freqpenta_tcem + thymopscore, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef[c("log_freqpenta_tcem","thymopscore"), c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m5"))

modelglm = glm(formula = trig~pentamer_expression_group2 + thymopscore, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef[c("pentamer_expression_group2medium","thymopscore"), c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m6"))

modelglm = glm(formula = trig ~ log_freqpenta_tcem + pentamer_expression_group2 + thymopscore, family = "binomial", data = ligands_nonhuman)
summary(modelglm)$coef[c("log_freqpenta_tcem","pentamer_expression_group2medium","thymopscore"), c("Estimate","Pr(>|z|)")]
summary(modelglm)$aic
save(modelglm, file = paste0("glm", ds, "/m7"))

modelfiles = list.files(paste0("glm", ds, "/"))
...
for(i in 1:nrow(resanova)) {
  load(paste0("glm", ds, "/", resanova$model1[i]))
  modelglm1 = modelglm
  load(paste0("glm", ds, "/", resanova$model2[i]))
...
save(resanova, file = paste0("objects/resanova_", ds))
load(paste0("objects/resanova_", ds))
```

#FigS8 Forest plot hydrophobicity
...
ligands_nonhuman$trig = as.numeric(as.character(ligands_nonhuman$trig))
load("../objects/hydroph_indices") #ebből egy vektor kell csak
indices = indices[27,]
ligands_nonhuman$hydrophobicity =  unname(sapply(ligands_nonhuman$tcem, function(y) {median(as.numeric(indices[,strsplit(y, "")[[1]]]))}))
...
ligands_nonhuman %<>% 
  ...
  dplyr::select(allele, epitope, trig, log_freqpenta_tcem, pentamer_expression_group2, thymo_zscore, hydrophobicity) %>% 
  na.omit()
#347 peptides removes from D1

##Model
colnames(ligands_nonhuman)[4:7] = c("TCEM frequency (log)", "TCEM expression group", "TCEM thymoproteasomal cleavage score (z-score)", "TCEM hydrophobicity")

modelglm_d1 = glm(formula = trig ~ `TCEM frequency (log)` + `TCEM expression group` + `TCEM thymoproteasomal cleavage score (z-score)` + `TCEM hydrophobicity`, family = "binomial", data = ligands_nonhuman)
summary(modelglm_d1)
...
modelglm_d2 = glm(formula = trig ~ `TCEM frequency (log)` + `TCEM expression group` + `TCEM thymoproteasomal cleavage score (z-score)` + `TCEM hydrophobicity`, family = "binomial", data = ligands_nonhuman)
summary(modelglm_d2)
...
fig_forest= grid.arrange(textGrob("Dataset 1"), fig_forest_d1, textGrob("Dataset 2"), fig_forest_d2, layout_matrix = rbind(c(1,1,1,1),c(2,2,2,2),c(2,2,2,2),c(2,2,2,2),c(2,2,2,2),c(2,2,2,2),c(3,3,3,3),c(4,4,4,4),c(4,4,4,4),c(4,4,4,4),c(4,4,4,4),c(4,4,4,4)))

ggsave(fig_forest, filename = "supp_plots_rs/figS8_forestplot_13.png", width = 13, height = 8, units = "cm", dpi = "retina")
```

#FigS9 Secondary anchors
##Create datasets
...
ligands_nonhuman$excluded = FALSE
for(i in 1:nrow(ligands_nonhuman)) {
  if(ligands_nonhuman$allele[i] %in% secanc_alleles) {
    other_alleles = setdiff(activating$allele[activating$epitope == ligands_nonhuman$epitope[i]],ligands_nonhuman$allele[i])
    if(length(other_alleles) == 0) {
      ligands_nonhuman$excluded[i] = TRUE
...
table(ligands_nonhuman$excluded)
ligands_nonhuman %<>% filter(excluded == FALSE)
save(ligands_nonhuman, file = "ligands_nonhuman_d1_wosecanc")
dataset_1 = cbind(ligands_nonhuman[,c("allele", "epitope", "trig", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d1")
...
table(ligands_nonhuman$excluded)
ligands_nonhuman %<>% filter(excluded == FALSE)
save(ligands_nonhuman, file = "ligands_nonhuman_d2_wosecanc")
dataset_2 = cbind(ligands_nonhuman[,c("allele", "epitope", "trig", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d2")

ligands_nonhuman = rbind(dataset_1, dataset_2)
rm(dataset_1, dataset_2)
ligands_nonhuman %<>% 
    mutate(group = case_when(
    .$dataset == "d1" & .$trig == 0 ~ "d1_nim",
    .$dataset == "d1" & .$trig == 1 ~ "d1_im",
    .$dataset == "d2" & .$trig == 0 ~ "d2_nim",
    .$dataset == "d2" & .$trig == 1 ~ "d2_im",
  )) %>% 
...
```{r}
load("ligands_nonhuman_d1d2_wosecanc")
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d2"))

table(ligands_nonhuman$group)

figS9_1 = ggplot(ligands_nonhuman, aes(x = group, y = freqpenta_tcem, color = trig, fill = trig)) + 
  geom_boxplot(outlier.shape = NA, lwd = .5, width = .6) +
  scale_x_discrete(labels = c("+","-","+","-")) +
  coord_cartesian(ylim = c(0,170)) +
  scale_y_continuous(trans = "pseudo_log", breaks = c(0,3,4,5,10,100)) +
...
```{r}
load("ligands_nonhuman_d1_wosecanc")
ligands_nonhuman %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman$trig)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
plotdata = plsmo(x = ligands_nonhuman$expperc, y = ligands_nonhuman$trig)
...
```{r}
load("ligands_nonhuman_d2_wosecanc")
ligands_nonhuman %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman$trig)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
...
figS9_45
rm(figS9_4, figS9_5, ligands_nonhuman)
```
...
figS9_6 = ggplot(ligands_nonhuman, aes(x = group, y = thymopscore, color = trig, fill = trig)) + 
  geom_boxplot(outlier.shape = NA, lwd = .5, width = .6) +
  scale_x_discrete(labels = c("+","-","+","-")) +
  coord_cartesian(ylim = c(.75,1.25)) +
  scale_color_manual(values = c("#DC0000B2","#3C5488B2")) +
...
figS9_7 = ggplot(ligands_nonhuman, aes(x = group, y = immunopscore, color = trig, fill = trig)) + 
  geom_boxplot(outlier.shape = NA, lwd = .5, width = .6) +
  scale_x_discrete(labels = c("+","-","+","-")) +
  coord_cartesian(ylim = c(.55,1.45)) +
  scale_color_manual(values = c("#DC0000B2","#3C5488B2")) +
...
#ggsave(figS9, filename = "FigS9_secanc.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
```
...
```{r}
chowell = read.delim("../objects/Chowell_st_ie.txt", sep = "\t", stringsAsFactors = F) #9888
load("ligands_nonhuman_d1d2")
chowell = chowell[chowell$dissimilarity > 0,] #4263
chowell = chowell[is.na(fmatch(chowell$nmer, ligands_nonhuman$epitope)),] #3430
rm(ligands_nonhuman)
...
figS10_chowell = ggplot(groups, aes(group_num, ratio)) +
  geom_point(size = 2) +
  ...
  labs(x = "Dissimilarity value range of groups", y = "Ratio of immunogenic and non-immunogenic peptides") +
  ...
  theme(
    ...
    axis.line = element_line(colour = "black"))
figS10_chowell
#ggsave(figS10_chowell, filename = "figS10.png", width = 11, height = 11, units = "cm", dpi = "retina")
rm(chowell, groups, b, b_exp, wm)
```

#FigS11 Cross-reactivity
...
#A6
tbl = read.delim("../crossreact/A6_position_depletion_matrix.csv", sep = ",", stringsAsFactors = F)
...
plot(tbl$dist, tbl$value)
rcorr(tbl$dist, tbl$value, type = "spearman")
rcorr(tbl$dist, tbl$value, type = "spearman")$P
boxplot(tbl$value ~ tbl$dist > 0.8)
wilcox.test(tbl$value ~ tbl$dist > 0.8)
...
ggsave(figs11, filename = "supp_plots_rs/FigS11.png", width = 22, height = 11, units = "cm", dpi = "retina")
...
table(dataset$group,dataset$trig)
fisher.test(dataset$group, dataset$trig)

#EXPR
dataset = ligands_nonhuman %>% filter(!is.na(pentamer_expression))
...
table(dataset$group,dataset$trig)
fisher.test(dataset$group[dataset$group %in% c(1,2)], dataset$trig[dataset$group %in% c(1,2)])

#THYMO
dataset = ligands_nonhuman %>% filter(!is.na(thymopscore))
...
#TCEMFREQ & THYMO
dataset = ligands_nonhuman %>% filter(!is.na(freqpenta_tcem),!is.na(thymopscore))
dataset$group = NA
...
#EXPR & THYMO
dataset = ligands_nonhuman %>% filter(!is.na(pentamer_expression),!is.na(thymopscore))
dataset$group = NA
...
```{r}
resdf = expand.grid(ds = c("d1", "d2", "d1d2"), type = c("epitope", "tcem"), excludedaa = rownames(AABLOSUM62), stringsAsFactors = F)
resdf %<>% mutate(n = NA, n_im = NA, n_nim = NA,
                  freqpenta_med = NA, freqpenta_med_im = NA, freqpenta_med_nim = NA, freqpenta_wilp = NA, freqpenta_fishp = NA, freqpenta_fishor = NA,
                  expr_fishp1 = NA, expr_fishor1 = NA, expr_fishp2 = NA, expr_fishor2 = NA,
                  thymopscore_med_im = NA, thymopscore_med_nim = NA, thymopscore_wilp = NA, thymopscore_fishp = NA, thymopscore_fishor = NA,
                  immunopscore_med_im = NA, immunopscore_med_nim = NA, immunopscore_wilp = NA, immunopscore_fishp = NA, immunopscore_fishor = NA)
for(i in 1:nrow(resdf)) {
  print(i)
  load(paste0("ligands_nonhuman_", resdf$ds[i]))
  if(resdf$type[i] == "epitope") {ligands_nonhuman %<>% filter(!grepl(resdf$excludedaa[i], epitope))} else {ligands_nonhuman %<>% filter(!grepl(resdf$excludedaa[i], tcem))}
  resdf$n[i] = ligands_nonhuman %>% nrow()
  resdf$n_im[i] = ligands_nonhuman %>% filter(trig == 1) %>% nrow()
  resdf$n_nim[i] = ligands_nonhuman %>% filter(trig == 0) %>% nrow()
  #TCEM FREQUENCY
  resdf$freqpenta_med[i] = ligands_nonhuman %>% pull(freqpenta_tcem) %>% median()
  resdf$freqpenta_med_im[i] = ligands_nonhuman %>% filter(trig == 1) %>% pull(freqpenta_tcem) %>% median()
  resdf$freqpenta_med_nim[i] = ligands_nonhuman %>% filter(trig == 0) %>% pull(freqpenta_tcem) %>% median()
  resdf$freqpenta_wilp[i] = wilcox.test(freqpenta_tcem ~ trig, ligands_nonhuman)$p.value
  ligands_nonhuman$freqpenta_tcem_group = cut(x = ligands_nonhuman$freqpenta_tcem, breaks = c(min(ligands_nonhuman$freqpenta_tcem),median(ligands_nonhuman$freqpenta_tcem),max(ligands_nonhuman$freqpenta_tcem)), labels = F, include.lowest = T, right = F)
  ft1 = fisher.test(matrix(c(
    ligands_nonhuman %>% filter(trig == 0, freqpenta_tcem_group == 2) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, freqpenta_tcem_group == 2) %>% nrow(), 
    ligands_nonhuman %>% filter(trig == 0, freqpenta_tcem_group == 1) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, freqpenta_tcem_group == 1) %>% nrow()),
    nrow = 2, ncol = 2))
  resdf$freqpenta_fishp[i] = ft1$p.value
  resdf$freqpenta_fishor[i] = ft1$estimate
  #EXPRESSION
  ligands_nonhuman$pentamer_expression_group = cut(x = ligands_nonhuman$pentamer_expression, breaks = c(min(ligands_nonhuman$pentamer_expression,na.rm=T),quantile(ligands_nonhuman$pentamer_expression,c(0.15,0.75),na.rm=T),max(ligands_nonhuman$pentamer_expression,na.rm=T)), labels = F, include.lowest = T, right = F)
  ft2 = fisher.test(matrix(c(
    ligands_nonhuman %>% filter(trig == 0, pentamer_expression_group == 2, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, pentamer_expression_group == 2, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(), 
    ligands_nonhuman %>% filter(trig == 0, pentamer_expression_group == 1, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, pentamer_expression_group == 1, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow()),
    nrow = 2, ncol = 2))
  resdf$expr_fishp1[i] = ft2$p.value
  resdf$expr_fishor1[i] = ft2$estimate
  ft3 = fisher.test(matrix(c(
    ligands_nonhuman %>% filter(trig == 0, pentamer_expression_group == 2, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, pentamer_expression_group == 2, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(), 
    ligands_nonhuman %>% filter(trig == 0, pentamer_expression_group == 3, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, pentamer_expression_group == 3, !is.na(trig), !is.na(pentamer_expression_group)) %>% nrow()),
    nrow = 2, ncol = 2))
  resdf$expr_fishp2[i] = ft3$p.value
  resdf$expr_fishor2[i] = ft3$estimate
  #THYMOPSCORE
  ligands_nonhuman$thymopscore_group = cut(x = ligands_nonhuman$thymopscore, breaks = quantile(ligands_nonhuman$thymopscore,c(0,0.25,1),na.rm = T), labels = F, include.lowest = T, right = F)
  resdf$thymopscore_med_im[i] = ligands_nonhuman %>% filter(trig == 1) %>% pull(thymopscore) %>% median(na.rm = T)
  resdf$thymopscore_med_nim[i] = ligands_nonhuman %>% filter(trig == 0) %>% pull(thymopscore) %>% median(na.rm = T)
  resdf$thymopscore_wilp[i] = wilcox.test(thymopscore ~ trig, ligands_nonhuman)$p.value
  ft4 = fisher.test(matrix(c(
    ligands_nonhuman %>% filter(trig == 0, thymopscore_group == 2) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, thymopscore_group == 2) %>% nrow(), 
    ligands_nonhuman %>% filter(trig == 0, thymopscore_group == 1) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, thymopscore_group == 1) %>% nrow()),
    nrow = 2, ncol = 2))
  resdf$thymopscore_fishp[i] = ft4$p.value
  resdf$thymopscore_fishor[i] = ft4$estimate
  #IMMUNOPSCORE
  ligands_nonhuman$immunopscore_group = cut(x = ligands_nonhuman$immunopscore, breaks = quantile(ligands_nonhuman$immunopscore,c(0,0.25,1),na.rm = T), labels = F, include.lowest = T, right = F)
  resdf$immunopscore_med_im[i] = ligands_nonhuman %>% filter(trig == 1) %>% pull(immunopscore) %>% median(na.rm = T)
  resdf$immunopscore_med_nim[i] = ligands_nonhuman %>% filter(trig == 0) %>% pull(immunopscore) %>% median(na.rm = T)
  resdf$immunopscore_wilp[i] = wilcox.test(immunopscore ~ trig, ligands_nonhuman)$p.value
  ft5 = fisher.test(matrix(c(
    ligands_nonhuman %>% filter(trig == 0, immunopscore_group == 2) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, immunopscore_group == 2) %>% nrow(), 
    ligands_nonhuman %>% filter(trig == 0, immunopscore_group == 1) %>% nrow(),
    ligands_nonhuman %>% filter(trig == 1, immunopscore_group == 1) %>% nrow()),
    nrow = 2, ncol = 2))
  resdf$immunopscore_fishp[i] = ft5$p.value
  resdf$immunopscore_fishor[i] = ft5$estimate
  rm(ft1, ft2, ft3, ft4, ft5, ligands_nonhuman)
}
aalevels = resdf %>% filter(ds == "d2", type == "tcem") %>% arrange(n) %>% pull(excludedaa)
resdf$excludedaa = factor(resdf$excludedaa, levels = aalevels)
resdf$ds = factor(resdf$ds, levels = c("d1", "d2", "d1d2"))

resdf %<>% mutate_at(7:26, function(x) signif(x, digits = 3))
#write.table(resdf, file = "results_when_aa_excluded.txt", quote = F, sep = "\t", dec = ".", row.names = F)
