...
```{r}
setwd("D:/CloudStation/pentamer_project/PNAS_revision/")
library(ggplot2)
library(magrittr)
library(dplyr)
library(Hmisc)
...
mypal
# show_col(mypal)
# #expression(italic(P) == 7.36 ~ '*' ~ 10^{-4})
# #expression(italic(P) == 0.004)
# rm(mypal)
...
```{r}
load("objects/ligands_nonhuman_d1d2")
...
ggsave(fig2, filename = "main_plots_rs/Fig2_9.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
...
```{r}
load("21out/ligands_nonhuman_d1d2_wosecanc")
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d1"))
wilcox.test(freqpenta_tcem ~ trig, subset(ligands_nonhuman, dataset == "d2"))
...
```{r}
load("21out/ligands_nonhuman_d1_wosecanc")
ligands_nonhuman %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman$trig)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
plotdata = plsmo(x = ligands_nonhuman$expperc, y = ligands_nonhuman$trig)
...
```{r}
load("21out/ligands_nonhuman_d2_wosecanc")
ligands_nonhuman %<>% filter(!is.na(pentamer_expression))
table(ligands_nonhuman$trig)
ligands_nonhuman$expperc = ecdf(ligands_nonhuman$pentamer_expression)(ligands_nonhuman$pentamer_expression)
...
ggsave(figS9, filename = "supp_plots_rs/FigS9_secanc.pdf", width = 22, height = 10, units = "cm", dpi = "retina")
```
...
figS10_chowell = ggplot(groups, aes(group_num, ratio)) +
  ...
  theme(
    ...
    axis.line = element_line(colour = "black"))
figS10_chowell
ggsave(figS10_chowell, filename = "supp_plots_rs/figS10.png", width = 11, height = 11, units = "cm", dpi = "retina")
