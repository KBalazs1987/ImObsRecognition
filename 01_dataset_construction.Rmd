...
EZ a lépés a kötési értékeket adja hozzá.
...
ligands_raw_d1 %<>% filter(!peptide %in% drop_eps) #no. of peptides after filtering: 4,089
rm(drop_eps)

#Epitope unique
eps = unique(ligands_raw_d1$peptide)
...
##Combine with T cell activation assays
...
##Filter 1 (binding assay, T cell activation assay, binding ratio)
...
ligands_raw_d2 %<>% filter(!peptide %in% drop_eps) #no. of peptides after filtering: 1,115
rm(drop_eps)

#Epitope unique
eps = unique(ligands_raw_d2$peptide)
for(j in 1:length(eps)) {
