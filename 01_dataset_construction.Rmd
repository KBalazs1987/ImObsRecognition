...
Predicting HLA-binding with NetMHCpan-4.0. Selecting pairs with either < 500 nM binding affinity or 
< 2% binding rank percentile
...
ligands_raw_d1 = activating %>% 
  ...
  dplyr::select(allele, peptide, n_activating_assay, activating_ratio) #no. of peptides after filtering: 21,474
```
...
```{r}
ligands_raw_d1 %<>% 
  filter(n_activating_assay >= 2) #no. of peptides after filtering: 5,630
```
...
ligands_raw_d1 %<>% 
  filter(!peptide %in% humaneps) #no. of peptides after filtering: 4,634
...
ligands_raw_d1 %<>% na.omit() #no. of peptides after filtering: 4,118
...
#writeLines(drop_peps, "01out/dropped_peptides.txt")
ligands_raw_d1 %<>% filter(!peptide %in% drop_peps) #no. of peptides after filtering: 4,089 #comment: false value on FigS1
rm(drop_peps)
```
...
```{r}
peps = unique(ligands_raw_d1$peptide) #no. of unique peptides: 3955
...
rm(peps, tempdf, j)
ligands_raw_d1 = unique(ligands_raw_d1) #no. of peptides after filtering: 3,955
...
ligands_raw_d2 = binding %>% 
  ...
  filter(!is.na(activating_ratio)) #no. of peptides after filtering: 6936
rm(activating, binding)
ligands_raw_d2 %<>% 
  filter(n_binding_assay >= 2, binding_ratio > 0.6) #no. of peptides after filtering: 2,345
```
...
```{r}
ligands_raw_d2 %<>% 
  filter(n_activating_assay >= 2) #no. of peptides after filtering: 1,506
```
...
humaneps = tempdf$peptide[tempdf$human == T]
ligands_raw_d2 %<>% filter(!peptide %in% humaneps) #no. of peptides after filtering: 1,318
rm(proteome, tempdf, humaneps)
```
...
ligands_raw_d2 = ligands_raw_d2[!is.na(ligands_raw_d2$immunogenicity),] #no. of peptides after filtering: 1,117
...
drop_peps = ligands_raw_d2 %>% dplyr::group_by(peptide) %>% dplyr::summarise(m = mean(immunogenicity)) %>% filter(m > 0, m < 1) %>% pull(peptide) #1 peptide: YAQMWSLMY
ligands_raw_d2 %<>% filter(!peptide %in% drop_peps) #no. of peptides after filtering: 1,115
rm(drop_peps)
```
...
```{r}
peps = unique(ligands_raw_d2$peptide) #unique peptides
for(j in 1:length(peps)) {
...
rm(peps, tempdf, j)
ligands_raw_d2 = unique(ligands_raw_d2) #no. of peptides after filtering: 1,087
...
ligands_nonhuman_d2 = ligands_raw_d2 %<>% 
  filter(peptide %in% rownames(dist_matr)) #no. of peptides after filtering: 635
...
```{r}
ligands_nonhuman_d1 = ligands_raw_d1[!ligands_raw_d1$peptide %in% ligands_nonhuman_d2$peptide,] #no. of peptides after filtering: 3,380
