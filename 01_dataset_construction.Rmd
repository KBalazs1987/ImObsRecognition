---
title: "Dataset 1 and 2 modification - PNAS"
author: "Balazs Koncz"
date: '2021 04 09 '
...
```{r}
source("D:/CloudStation/mygit_EXT/ImObsRecognition/objects/clustal_omega_function.R")
...
#writeLines(dropped_peps_clo, "01out/dropped_eps_clo_d2.txt")
rm(dropped_peps_clo)
...
#Create datasets for supplementary

```{r}
load("objects/ligands_nonhuman_d1")
dataset_1 = cbind(ligands_nonhuman[,c("allele", "peptide", "tcem", "immunogenicity", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d1")
load("objects/ligands_nonhuman_d2")
dataset_2 = cbind(ligands_nonhuman[,c("allele", "peptide", "tcem", "immunogenicity", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d2")
rm(ligands_nonhuman)

#For FigS4 housekeeping ratio - pentamer expression group
nb_of_groups = 20
dataset_1$pentamer_expression_group = cut(dataset_1$pentamer_expression, unique(quantile(dataset_1$pentamer_expression, seq(0,1,1/nb_of_groups), na.rm = T)), include.lowest = T, labels = F, right = F)
table(dataset_1$pentamer_expression_group)
dataset_2$pentamer_expression_group = cut(dataset_2$pentamer_expression, unique(quantile(dataset_2$pentamer_expression, seq(0,1,1/nb_of_groups), na.rm = T)), include.lowest = T, labels = F, right = F)
table(dataset_2$pentamer_expression_group)
rm(nb_of_groups)

#For Fig3B similarity group
datasets = rbind(dataset_1, dataset_2)
datasets$bl62score_group = cut(datasets$bl62score, 25)
table(datasets$bl62score_group)

#For FigS seconday anchors
load("01out/activating_d1")
...
write.table(ligands_nonhuman, file = "01out/ligands_nonhuman_d1.txt", quote = F, sep = "\t", dec = ".", row.names = F)
...
ligands_nonhuman %<>% mutate_at(c(5,7,8,9,11), function(x) round(x,3))
write.table(ligands_nonhuman, file = "01out/ligands_nonhuman_d2.txt", quote = F, sep = "\t", dec = ".", row.names = F)
...
```{r}
load("objects/ligands_nonhuman_d1")
dataset_1 = cbind(ligands_nonhuman[,c("allele", "peptide", "immunogenicity", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d1")
load("objects/ligands_nonhuman_d2")
dataset_2 = cbind(ligands_nonhuman[,c("allele", "peptide", "immunogenicity", "tcem", "freqpenta_tcem", "pentamer_expression", "thymopscore", "immunopscore", "bl62score", "housekeeping_ratio")], dataset = "d2")
...
save(ligands_nonhuman, file = "objects/ligands_nonhuman_d1d2") #4015
```
...
ligands_nonhuman %>% group_by(immunogenicity) %>% summarize(m = median(freqpenta_tcem))
wilcox.test(freqpenta_tcem ~ immunogenicity, ligands_nonhuman)
...
fisher.test(matrix(c(
  ligands_nonhuman %>% filter(immunogenicity == 0, freqpenta_tcem_group == 2) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, freqpenta_tcem_group == 2) %>% nrow(), 
  ligands_nonhuman %>% filter(immunogenicity == 0, freqpenta_tcem_group == 1) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, freqpenta_tcem_group == 1) %>% nrow()),
  nrow = 2, ncol = 2))
...
fisher.test(matrix(c(
  ligands_nonhuman %>% filter(immunogenicity == 0, pentamer_expression_group == 2, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, pentamer_expression_group == 2, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(), 
  ligands_nonhuman %>% filter(immunogenicity == 0, pentamer_expression_group == 1, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, pentamer_expression_group == 1, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow()),
  nrow = 2, ncol = 2))
fisher.test(matrix(c(
  ligands_nonhuman %>% filter(immunogenicity == 0, pentamer_expression_group == 2, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, pentamer_expression_group == 2, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(), 
  ligands_nonhuman %>% filter(immunogenicity == 0, pentamer_expression_group == 3, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow(),
  ligands_nonhuman %>% filter(immunogenicity == 1, pentamer_expression_group == 3, !is.na(immunogenicity), !is.na(pentamer_expression_group)) %>% nrow()),
  nrow = 2, ncol = 2))
...
wilcox.test(thymopscore ~ immunogenicity, ligands_nonhuman)
fisher.test(matrix(c(
...
wilcox.test(immunopscore ~ immunogenicity, ligands_nonhuman)
fisher.test(matrix(c(
